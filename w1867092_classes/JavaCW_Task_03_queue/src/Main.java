...
public class Main {
    ...
    public static void cruiseMenu(Cabin[] cabinList,CircularQueue queue){                    // Cruise Ship Passenger controller menu
        ...
        switch (GetInput){
            ...
            case "e": DisplayEmptyCabins(cabinList);
                break;
            case "d": DeleteCustomerFromCabin(cabinList, queue);
                break;
    ...
    private static boolean full(Cabin[] cabinArray) {                                                   // check all cabins ara full
    ...
    private static void addCustomer(Cabin[] cabinArray, CircularQueue queue) {                            // add Customer to cabin
        ...
        while (true){
            try{
                ...
                } else if (0 <= cabinNo && cabinNo < 12 && !cabinArray[cabinNo].Full()){
                    int passengerNo = 1;
                    while (true) {
    ...
    public static void DeleteCustomerFromCabin(Cabin[] cabins, CircularQueue queue) {                // Delete Customer From Cabin
        Scanner scanner = new Scanner(System.in);
            ...
            for (int j = 0; j < 3; j++) {
                if(cabins[i].getCustomerList()[j].getFirstName().toLowerCase(Locale.ROOT).equals(input.toLowerCase(Locale.ROOT))) {
                    ...
                    if(deQueuedPassenger != null) {
                        cabins[i].getCustomerList()[j].setFirstName(deQueuedPassenger.getFirstName());
                        cabins[i].getCustomerList()[j].setSurname(deQueuedPassenger.getSurname());
                        cabins[i].getCustomerList()[j].setExpenses(deQueuedPassenger.getExpenses());
